...
class LinkedList(object):
    ...
    def length(self):
        ...
        Best and worst case running time:O(n), always runs through each item"""
    ...
    def get_at_index(self, index):
        ...
        Worst case running time: O(n), last index"""
    ...
    def insert_at_index(self, index, item):
        ...
        raise ValueError if the given index is out of range of the list size.
        Best case running time: O(1), index 0
        Worst case running time: O(n), second-to-last index"""
        # Check if the given index is out of range and if so raise an error
        if not (0 <= index <= self.size):
            raise ValueError('List index out of range: {}'.format(index))
    ...
    def append(self, item):
        ...
        Best and worst case running time: O(1), stored self.tail"""
    ...
    def prepend(self, item):
        ...
        Best and worst case running time: O(1), stored self.head"""
        # Create a new node to hold the given item
        new_node = Node(item)
        # Check if this linked list is empty
        if self.is_empty():
    ...
    def replace(self, old_item, new_item):
        ...
        Worst case running time: O(n) if last item"""
        ...
        while node is not None:  # Up to n iterations if we don't exit early
            # Check if this node's data satisfies the given quality function
            if old_item == node.data:  # Constant time to call quality function
                # We found data satisfying the quality function, so exit early
                node.data = new_item  # Constant time to reassign data
                return
            # Skip to the next node
            node = node.next  # Constant time to reassign a variable
        # Item not found
